...
void presenttime()
{
    ...
    printf("Time : %4d.%d.%d  %d:%d:%d", t.tm_year + 1900, t.tm_mon+1, t.tm_mday, t.tm_hour, t.tm_min, t.tm_sec);
...
void to_write(info *arr, int *tnum, int *osnum)
{
    char tmp[200];
    char ch;
    int flag = 0;
    ...
    strncpy(arr[*osnum].memo[tnum[*osnum]], tmp, sizeof(tmp));
    flag = 0;
...
void to_view(info *arr, int *pnum, int *tnum)
{
    int i, j;
